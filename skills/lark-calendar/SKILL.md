...
本技能的部分命令需要 **用户 open_id**（`ou_xxx`）或 **群聊 chat_id**（`oc_xxx`）作为入参（如 `--user-id`、`--attendee-ids`）。当上下文中没有现成的 ID，需要通过搜索来获取时：

- **搜索用户获取 open_id**：参考 [lark-contact](../lark-contact/SKILL.md) 技能，使用 `lark-cli contact +search-user` 按姓名或邮箱搜索用户。
- **搜索群聊获取 chat_id**：参考 [lark-im](../lark-im/SKILL.md) 技能，使用 `lark-cli im +chat-search` 按群名关键词搜索群聊。
