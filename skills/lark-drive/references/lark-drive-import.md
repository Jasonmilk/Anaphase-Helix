...
将文件（如 Word、TXT、Markdown、Excel 等）导入并转换为飞书在线云文档（docx、sheet、bitable）。底层统一通过 `POST /open-apis/drive/v1/import_tasks` 接口创建导入任务，并在 shortcut 内做有限次数轮询 `GET /open-apis/drive/v1/import_tasks/:ticket`。

## 命令

```bash
# 导入 Markdown 为新版文档 (docx)，通过 URL 上传
lark-cli drive +import --file https://example.com/README.md --type docx

# 导入 Excel 为电子表格 (sheet)
lark-cli drive +import --file https://example.com/data.xlsx --type sheet

# 导入到指定文件夹，并指定导入后的文件名
lark-cli drive +import --file https://example.com/data.csv --type bitable --folder-token <FOLDER_TOKEN> --name "导入数据表"

# 预览底层调用链（上传 -> 创建任务 -> 轮询）
lark-cli drive +import --file https://example.com/README.md --type docx --dry-run
```
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--file` | 是 | 文件路径，根据文件后缀名自动推断 `file_extension`；文件需满足对应格式的导入大小限制，超过 20MB 且仍在允许范围内时会自动切换分片上传 |
...
| `--name` | 否 | 导入后的在线云文档名称，不传默认使用文件名去掉扩展名后的结果 |
...
- **完整执行流程**：此 shortcut 内部封装了完整流程：
  ...
  2. 调用 `import_tasks` 接口发起导入任务，自动根据文件提取扩展名并构造挂载点（`mount_point`）参数
...
除扩展名与目标类型匹配外，`drive +import` 还会在上传前校验格式级大小限制：
