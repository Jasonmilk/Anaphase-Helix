...
```typescript
{
  ...
  gap: number;                   // 必填，子节点间距
  padding: number | [number, number] | [number, number, number, number]; // 必填
...
```json
{
  ...
  "children": [
    { "type": "svg", "x": 0, "y": 0, "width": 1400, "height": 680,
      "svg": { "code": "<svg viewBox=\"0 0 1400 680\" ...>...</svg>" } },
...
{ "type": "svg", "width": 32, "height": 32,
  "svg": { "code": "<svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"#3B82F6\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"10\"/><polyline points=\"12 6 12 12 16 14\"/></svg>" } }
```
...
`text` 和 `content` 中出现的双引号必须写成 `\"`，这是 JSON 规范要求。
