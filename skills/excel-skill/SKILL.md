...
- **表格创建 vs 其他场景**：当用户没有现成数据文件、需要从零创建 Excel（如做模板、整理信息成表格）时走此路由；若创建后还需分析/建模，则创建完成后再按对应场景路由继续

> **多场景叠加**：若任务横跨多个场景，按主诉求选主路由，次要场景的相关说明也可参考对应 reference 文件。
...
3. **公式写在最终交付表格里**：凡是需要计算，优先写 Excel 公式到最终交付给用户的表格对应单元格（原 sheet 或用户要求的输出 sheet）；
**⚠️ 关键：Excel 公式永远是第一选择**
...
| 情况 | 处理方式 |
|------|---------|
| 用户上传了 Excel/CSV 文件 | 直接进入第二步 |
| 用户口述/粘贴了数据但没有文件 | 先将信息结构化为 DataFrame，直接按需求创建 Excel；表格排版（含行高/列宽/截断）按 `references/ref-xlsx-workflow.md` 执行 | 针对生成的Excel如果有进一步数据洞察等需求，可以进入第二步继续优化修改
...
1. **运行探表脚本**了解数据结构（注意probe_excel只是方便了解数据的探测函数，具体执行任务必须使用FileView了解文件完整的内容）：
   - Excel 文件：`python scripts/probe_excel.py <文件路径>`
...
2. **评估数据规模**，决定执行策略：
   ...
   - **多 sheet 文件**：逐个 sheet 单独执行完整的「探表 → 处理 → 输出」流程

3. **识别合并单元格**：查看探表输出中的 `merged_cells` 字段，合并单元格的值仅存储在左上角；提取类任务需 `ffill` 填充；写公式/做合并时只允许在左上角单元格写值/公式，详见 `references/ref-xlsx-workflow.md`
...
5. **识别“特殊行/特殊单元格”并特殊处理**：若出现“合计/总计/小计/汇总/累计”等与明细口径不同的行：
   - 在探表输出的 `special_rows` 中定位这些行，并结合预览确认其是否为汇总行
...
读取路由决策表中对应的 reference 文件，按其中的工作流程执行任务。

### 第 4.5 步：透视表增强判定

在主路由任务执行完毕后、交付前，判断是否需要叠加透视表/透视图。**透视表是增强层，不改变主路由逻辑。**,如果预计或者检查发现透视表效果较差可以不生成。
...
2. 强制验证五项：原始数据完整性、透视数据准确性、图表有效性、公式重算零错误（recalc.py）、文件可用性
3. 任一验证失败且无法快速修复 → 删除透视 Sheet，回退到无透视版本交付，用 Canvas 展示透视数据作为替代

**工具链**：`ShellExec`（运行 Python 脚本）→ `open_url_in_browser` + `take_screenshot`（视觉校验）→ `CanvasCreateFile`（webview 上屏预览）→ `NotifyHuman`（文件交付）
...
| **公式错误** | 交付前必须零公式错误（`#REF!` `#DIV/0!` `#VALUE!` `#N/A` `#NAME?`），使用 `scripts/recalc.py` 验证 |
...
透视表是**附加增强能力**，在主路由任务完成后叠加，不改变原有路由逻辑。
...
4. **公式重算**：`python scripts/recalc.py output.xlsx` → total_errors == 0
...
| 步骤 | 工具 | 用途 |
|------|------|------|
| 预览数据 | `FileView` / `ShellExec` (probe_excel.py) | 快速了解数据结构和适合度 |
| 计算+格式化+图表 | `ShellExec` (Python 脚本) | pandas 聚合 → openpyxl 写入 → 图表生成 |
| 公式验证 | `ShellExec` (recalc.py) | 确认零公式错误 |
