...
get_playwright_npm_version() {
    # Try global first (with timeout to avoid hangs)
    local ver=$(timeout 10 npm list -g playwright 2>/dev/null | grep playwright@ | sed 's/.*@//' | head -1)
    if [[ -n "$ver" ]]; then
        echo "$ver"
        return
    fi
    # Try npx (with timeout)
    ver=$(timeout 10 npx playwright --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
    if [[ -n "$ver" ]]; then
        echo "$ver"
        return
    fi
...
get_expected_chromium_revision() {
    local output=$(timeout 15 npx playwright install chromium --dry-run 2>&1 || true)
...
verify_browser_usable() {
    ...
    local output
    output=$(timeout 15 node "$helper" 2>/dev/null)
    local code=$?

    if [[ $code -eq 124 ]]; then
        echo "error:timeout (playwright detection hung)"
        return
    fi
    ...
    if [[ -z "$output" ]]; then
        if [[ $code -eq 0 ]]; then
            ...
            echo "error:browser probe failed (exit $code)"
        fi
    else
        echo "$output"
    fi
