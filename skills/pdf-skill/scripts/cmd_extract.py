...
def extract_text(pdf_path: str, pages: str = None):
    ...
    if total_chars == 0:
        ...
        result["warning"] = (
            "该 PDF 未提取到任何文本内容，可能为扫描件（纯图片 PDF）。"
            "建议：1) 使用 OCR 工具（如 Tesseract）先进行文字识别；"
            "2) 或使用 extract image 命令提取图片后再处理。"
        )
    elif len(page_indices) > 0 and total_chars / len(page_indices) < 10:
        result["likely_scanned"] = True
        result["warning"] = (
            "该 PDF 每页提取的文本极少（平均 < 10 字符/页），"
            "可能包含大量图片或为部分扫描件。完整内容提取可能需要 OCR。"
        )
