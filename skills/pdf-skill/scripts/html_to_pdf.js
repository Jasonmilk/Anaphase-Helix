...
let chromium;
try {
  const helper = require('./browser_helper');
  const pw = helper.loadPlaywright();
  chromium = pw.chromium;
  var resolveChromium = helper.resolveChromium;
} catch (err) {
  console.error('ERROR: Failed to load Playwright.');
  console.error('  ' + (err.message || err));
  console.error('');
  console.error('Fix: run the following commands in the scripts/ directory:');
