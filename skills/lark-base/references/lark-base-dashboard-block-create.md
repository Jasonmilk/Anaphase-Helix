...
- **`data_config` 结构随 `type` 变化**，不同组件类型字段不同，**必须阅读 [dashboard-block-data-config.md](dashboard-block-data-config.md) 了解如何构造**
- **组件创建必须串行执行**，不能并发
...
| `--data-config <json>` | 否 | 数据配置 JSON，**结构随 type 变化**。**必须阅读 [dashboard-block-data-config.md](dashboard-block-data-config.md) 了解如何构造** |
