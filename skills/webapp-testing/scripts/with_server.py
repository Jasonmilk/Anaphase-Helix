...
def main():
    parser = argparse.ArgumentParser(description='使用一个或多个服务器运行命令')
    ...
    parser.add_argument('command', nargs=argparse.REMAINDER, help='服务器准备就绪后运行的命令')
    ...
    try:
        ...
        for i, server in enumerate(servers):
            ...
            # 等待此服务器准备就绪
            print(f"等待端口 {server['port']} 上的服务器...")
            if not is_server_ready(server['port'], timeout=args.timeout):
                raise RuntimeError(f"服务器未能在 {args.timeout} 秒内在端口 {server['port']} 上启动")
        ...
        # 运行命令
        print(f"运行: {' '.join(args.command)}\n")
        result = subprocess.run(args.command)
        sys.exit(result.returncode)

    finally:
        # 清理所有服务器
        print(f"\n停止 {len(server_processes)} 个服务器...")
