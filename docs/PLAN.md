...
## 当前阶段：候选 G 完成 + G-5 易用引导完成（`up` 首跑引导四段式：欢迎/前置检查/启动/下一步）——一条命令起全栈且首跑零困惑；下一步 G2 Web 面板 或 候选裁决
...
- **候选 G：Anaphase 驾驶舱**（完成：G-T2 ✅ / G-T3..T5 ✅ / G-T6 ✅ / G-3 transport 契约修复 ✅ / G-4 bootstrap ✅）——Cellrix 白盒驾驶舱（模式栏 + 经历时间线 + Ledger 审查视图 + 生态状态板）；TUI 先行，Web 面板（G2）后续；**bootstrap `up`（ADR-0011）：一条命令起全栈（tentacle→anaphase→探测→可选 --cockpit）**；**G-5 易用引导 UX 完成**（ADR-0012：欢迎/前置检查/启动/下一步四段式，缺失项带构建提示，首跑零困惑）
...
*Anaphase-Helix PLAN v2.4（候选 G 完成 + G-4/G-5，2026-09-06）*
