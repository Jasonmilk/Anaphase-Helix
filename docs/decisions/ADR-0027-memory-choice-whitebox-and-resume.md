...
- **状态**: Accepted
- **日期**: 2026-09-07
- **决策范围**: Anaphase（memory 元数据透传 / resume 续接）/ Cellrix（证轨甘特图 + SA-Core 白盒渲染 + 续聊按钮）
- **关联**: ADR-0026（会话事件流）、ADR-0025（L0-L3 全层实弹）、ADR-0023（on-demand 认知注入）
- **目标**: ①证轨（ProveTrack）能显示 Helix 从记忆里**捞了什么、信了几分**；②点击一段经历可以**继续这段对话**；③单轮时间线有 DSH 式甘特图，词汇保留 Helix 自有。

## 1. 背景与问题

ADR-0026 后，证轨能展示一轮的 8 事件（START…END），但有三处"黑盒"：
...
- **只写 id/tier/heat/phase，绝不写节点正文**（沿用 ADR-0026"写前脱敏 + 摘要-only"原则——证轨是白盒，不是敏感数据湖）。
...
- `context/inject` 写 `resume_from`（含摘要文本），证轨可见续接来源。
...
| 事件流写节点正文 | 证轨变敏感数据湖，违背 ADR-0026 脱敏原则；白盒展示 provenance 足够 |
...
**正面**：
- 证轨从"工具调用轨迹"升级为"认知决策轨迹"——看到 Helix 从记忆捞了什么、信了几分（heat）、处于什么相位（gas/liquid/crystal）；
...
> 全部摊在证轨里给人类看；点一段经历，Helix 还记得上一轮它说过什么。
