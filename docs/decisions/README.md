...
| 0042 | 400 行红线债务清偿 —— D11 的"另行开轮"到期 | Accepted | 2026-09-18 |
| 0043 | 特性自持落点 —— 新功能不得再默认落进 `main.rs` 与 `config.rs` | Proposed | 2026-09-20 |
| 0044 | 授权缝 —— 承诺与许可分离，许可来自人类措辞 | Active | 2026-09-20 |
...
`0031`–`0033` 属 helix-mind —— 那是**旧规则（生态共享序列）**下的产物。

> **⚠️ 2026-09-20：编号规则改为「每仓自增」。** 本文件上一版写的是
> 「ADR 编号是生态共享序列，不是每仓自增」—— **该规则已失效**，理由是它自己给出的证据：
>
> 1. **它没换来唯一性**：`ADR-0042` 被两仓同时持有 —— `helix-mind:ADR-0042`（2026-09-17）
>    与本仓 `anaphase:ADR-0042`（2026-09-18）。
> 2. **它一点都没多给**：既然跨仓引用本来就必须写仓名（`anaphase:ADR-0042`），
>    共享序列换来的「裸编号唯一」是**零收益**。
> 3. **分配下一个号要扫全生态**：为一个新 ADR 挑号，实测得扫全工作区 **95 个 ADR 文件**才敢定。
>    每仓自增下，`ls docs/decisions/` 即答。
> 4. **旧规则只写在本仓一个 README 里**（生态内其他仓未写）⇒ **一个仓里的规则管不住生态**
>    —— 与 K-002「双副本 / 单一事实源」同族，只是这次表现为**规则缺副本**。
>
> **⇒ 新规则**：下一个号 = **本仓**最大号 + 1；**跨仓引用一律写仓名**；**已分配编号不回改**
> （重写历史会打断全部交叉引用）。（既有例子保留：`anaphase:ADR-0016` 编排哲学
> ≠ `Cellrix:ADR-0016` 证轨资产解耦；`ADR-0017` 同理。）
> **生态层同一条已写入 `commonintents/.github:DNA.md §三`。**
> **⇒ 因此 `anaphase:ADR-0043` 与 `helix-mind:ADR-0043` 同号符合新规则** —— 键是 `仓:号`，
> 不是裸编号。**根因**：`DNA.md` 原文只写「按顺序递增」，**没说是哪一级的序列** ——
> **歧义写在规则里，就会在实现里分叉。**
